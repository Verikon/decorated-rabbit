	...
	return function (target) {
		...
		let WrappedRabbit = class WrappedRabbit extends target {
			...
			constructor(cargs) {
				...
				if (!RabbitInstances[instance] || !(RabbitInstances[instance].inst instanceof _DecoratedRabbit2.default)) {

					RabbitInstances[instance] = {
						inst: new _DecoratedRabbit2.default(provisioned_args),
						ids: [id]
					};
				} else {
			...
			async closeRabbit(args) {
				...
				RabbitInstances[instance].ids = RabbitInstances[instance].ids.filter(iid => {
					return iid !== id;
				});
...
const cte = exports.cte = function (options) {
